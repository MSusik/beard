...
def _parallel_fit(fit_, partial_fit_, estimator, verbose, data_queue,
    ...
    while status != 'end':
        ...
        if existing_clusterer and partial_fit_ and not fit_:
        ...
        try:
            ...
            # Something wrong happenned in the subprocess
            print(sys.exc_info()[0], file=sys.stderr)
            # Put everything in a one block
...
class BlockClustering(BaseEstimator, ClusterMixin):
    ...
    def _fit(self, X, y, blocks):
        ...
        for block in self._blocks(X, y, blocks):
            if presend > self.n_jobs and not skip:
            ...
            if len(X) == 1:
                clusterer = _SingleClustering()
                clusterer.fit(X)
                self.clusterers_[block[0]] = clusterer
                skip = True
                blocks_computed += 1
            else:
        ...
        for x in range(0, self.n_jobs):
            if blocks_computed < blocks_number:
                print("WAITING")
                b, clusterer = result_queue.get()
                blocks_computed += 1
                if clusterer:
